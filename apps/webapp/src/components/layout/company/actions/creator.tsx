...
function PushAction({ loading, onSubmit }: { loading: boolean; onSubmit: any }) {
  return (
    <div className="pt-2">
      {!loading ? (
        <button
...
function MessagingActionForm({ context }: { context: ActionsContext }) {
  ...
  const onSubmit = async (e: any) => {
    ...
    if (e.target.name === 'push') {
      ...
      await sendPush(
        signer as any,
        '[Update] ' + context.company.creator.address,
        data.get('message') as string,
      );
      console.log('pushing: ', data.get('message'), context.company.creator.address);
      setLoading2(false);
    } else {
      setLoading(true);
      const broadcast = context.company.status.investments.map((inv) => inv.investor.address);
      await broadcastMessage(signer, data.get('message') as string, broadcast);
    ...
    <form className="w-full">
      <div className="flex flex-col lg:flex-col lg:gap-1 w-full">
        ...
        <div className="flex flex-row lg:flex-row lg:gap-2">
          ...
          <div>
            <XMTPAction loading={loading} onSubmit={onSubmit} />
          </div>
          <div>
            <PushAction loading={loading2} onSubmit={onSubmit} />
          </div>
        </div>
      </div>
    </form>
